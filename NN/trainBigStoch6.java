...
public class trainBigStoch6
{
    ...
    public static double networkCost(double[][] trainData,double[][] label,double[][] hiddenBiases,double[][] hiddenWeights,double[][] finalBiases,double[][] finalWeights,boolean print) {
        ...
        hiddenOutputSig = softmax(hiddenOutput,hiddenBiases);
        ...
        hiddenBiasesGrad = hadMul(mul(finalBiasesGrad,finalWeights,true,false),dsoftmax(hiddenOutputSig));
    ...
    public static double[][][] dsoftmax(double a[][])
    {
        double[][][] x = new double[a.length][a[0].length][a[0].length];
        
        for(int i=0; i<a.length; i++){
            for(int j=0; j<a[0].length; j++){
                for(int k=0; k<a[0].length; k++){
                    if(j==k) {
                        x[i][j][k] = a[i][j]*(1-a[i][k]);
                    }
    ...
    public static double[][] hadMul(double[][] a, double[][][] b) {
        assert(b[0].length == b[0][0].length): "wrong size";
        assert(a[0].length == b[0].length): "wrong size";
        assert(a.length == b.length): "wrong size";
